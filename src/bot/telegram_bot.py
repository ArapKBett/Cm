...
class TelegramBot:
    ...
    def setup_handlers(self):
        ...
        dp.add_handler(CommandHandler("forex", self.analyze_forex))
    ...
    def analyze(self, update, context):
        ...
        if not isinstance(data, pd.DataFrame):
            update.message.reply_text(f"Error fetching data for {symbol}.")
            return
        ...
        context.bot.send_message(chat_id=Config.TELEGRAM_CHANNEL_ID, text=message, parse_mode="Markdown")
        context.bot.send_photo(chat_id=Config.TELEGRAM_CHANNEL_ID, photo=open(chart_path, "rb"))

    def analyze_forex(self, update, context):
        ...
        if not isinstance(data, pd.DataFrame):
            update.message.reply_text(f"Error fetching forex data for {pair}.")
            return
